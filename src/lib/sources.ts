...
import { SOURCES_DIR, credentialCachePath, sourceConfigPath, sourceDir, sourceGuidePath } from "./paths";
...
export function writeSourceConfig(slug: string, config: SourceConfig) {
  ...
  writeFileSync(sourceConfigPath(slug), JSON.stringify(config, null, 2), "utf-8");
}
...
export function readCredential(slug: string): string | null {
  ...
  try {
    const cache = JSON.parse(readFileSync(cachePath, "utf-8")) as CredentialCache;
...
export function writeCredential(slug: string, value: string, expiresAt?: number) {
  mkdirSync(sourceDir(slug), { recursive: true });
  const cache: CredentialCache = { value, ...(expiresAt ? { expiresAt } : {}) };
  writeFileSync(credentialCachePath(slug), JSON.stringify(cache, null, 2), "utf-8");
}
...
export function loadAllSources(): LoadedSource[] {
  return listSlugs()
    .map((slug) => {
      const config = readSourceConfig(slug);
      if (!config || !config.enabled) return null;
      const guide = readGuide(slug);
      const isAuthenticated = config.authType === "none" || readCredential(slug) !== null;
